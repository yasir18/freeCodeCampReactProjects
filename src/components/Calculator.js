...
const Calculator = () => {
  ...
  const handleClick = (e) => {
    ...
    if (key === "+" || key === "-" || key === "*" || key === "/") {
      ...
      tempValue === "0" ? setTempValue(key) : setTempValue(tempValue + key);
  ...
  const evaluateExpression = (str1, str2) => {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (i < len) {
        if (str[i] === "-" && (i === 0 || containsOperator(str[i - 1]))) {
          tempStr = "-";
        } else {
          //operator
          ...
          while (
            precedence(str[i]) <= precedence(operators[operators.length - 1])
          ) {
            let num1 = numbers.pop();
            let num2 = numbers.pop();
            console.log(`num2 ${num2} num1 ${num1}`);
            numbers.push(
              performOperation(num2, num1, operators[operators.length - 1])
            );
            operators.pop();
          }
